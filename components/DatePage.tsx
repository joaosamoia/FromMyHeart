...
// (R$19,90), com fluxo curto próprio no quiz.
//
// São DOIS layouts com o MESMO fluxo de 6 telas — só os tokens visuais mudam.
// Manter um único componente evita duplicar a lógica (envio pro WhatsApp,
// navegação, brincadeira do sim/não) em dois arquivos que teriam que ser
// corrigidos em dobro depois.

import { useState, useRef } from "react";

export type DateTemplateId = "sunset" | "ultravioleta";
...
type Tokens = {
  ...
  // Personagens por momento da história. Se o template não tiver imagem,
  // cai no emoji correspondente — assim nada quebra se um arquivo faltar.
  imgAbertura?: string; // tela 1 — pedindo (carinha triste)
  imgSim?: string; // tela 2 — comemorando (carinha com rosa)
  imgFinal?: string; // tela 6 — combinado (pinguim com flor)
...
export const DATE_TEMPLATES: Record<DateTemplateId, Tokens> = {
  sunset: {
    nome: "Pôr do sol",
    descricao: "Laranja quente, pinguim e caixa alta",
    bg: "radial-gradient(120% 120% at 50% 0%, #FF6A2B 0%, #F94C12 55%, #E03A07 100%)",
    cardBg: "rgba(28,10,4,.62)",
    cardBorder: "rgba(255,120,60,.35)",
    cardShadow: "0 18px 40px rgba(0,0,0,.32)",
    fieldBg: "rgba(20,6,2,.6)",
    text: "#FFF4EE",
    muted: "#FFD9C7",
    accent: "#FF5A1F",
    accentSoft: "rgba(255,120,60,.22)",
    btnBg: "linear-gradient(180deg,#FF7A3D,#FF5A1F)",
    btnText: "#fff",
    btnShadow: "0 10px 22px rgba(255,90,31,.5)",
    radius: 22,
    btnRadius: 40,
    ...
    titleWeight: 800,
    titleSpacing: 0.4,
    titleShadow: "0 2px 0 rgba(0,0,0,.35)",
    optionCols: 3,
    colorScheme: "dark",
    imgAbertura: "/date/sad-please.png",
    imgSim: "/date/flirty-rose.png",
    imgFinal: "/date/penguin-flower.png",
    emojiAbertura: "🥺",
    emojiSim: "🥰",
    emojiFinal: "🐧",
    confete: ["💙", "🖤", "🤎", "❤️", "💜"],
  ...
  // Roxo saturado, alto contraste — pensado pra gravar anúncio: cores fortes
  // e texto claro sobre fundo escuro seguram bem na compressão de vídeo.
  ultravioleta: {
    nome: "Ultravioleta",
    descricao: "Roxo vibrante, alto contraste",
    bg: "radial-gradient(120% 120% at 50% 0%, #B15CFF 0%, #7C3AED 55%, #4C1D95 100%)",
    cardBg: "rgba(24,8,42,.62)",
    cardBorder: "rgba(190,140,255,.35)",
    cardShadow: "0 18px 40px rgba(0,0,0,.36)",
    fieldBg: "rgba(18,6,32,.6)",
    text: "#F8F0FF",
    muted: "#E0CCFF",
    accent: "#C084FC",
    accentSoft: "rgba(192,132,252,.22)",
    btnBg: "linear-gradient(180deg,#CE9CFF,#A855F7)",
    btnText: "#fff",
    btnShadow: "0 10px 22px rgba(168,85,247,.5)",
    radius: 26,
    btnRadius: 99,
    font: "'Trebuchet MS','Segoe UI',system-ui,sans-serif",
    titleUpper: true,
    ...
    confete: ["💜", "🩷", "🤍", "💖", "✨"],
    coracao: "💜",
    // Fica false porque "multiply" só disfarça bem fundo claro sobre laranja.
    // Sobre roxo, um print de fundo laranja ficaria escuro e sujo — então aqui
    // as imagens precisam mesmo estar recortadas (PNG transparente).
    imgComFundo: false,
  },
};
...
export default function DatePage({
  ...
  const nick = nickname && nickname.trim() ? nickname.trim() : "gatinha";
  ...
  const confete = Array.from({ length: 14 }, (_, i) => ({
    ...
    delay: (i * 7) % 6,
    size: 16 + ((i * 5) % 14),
    e: T.confete[i % T.confete.length],
  }));
  ...
  const h1: React.CSSProperties = {
    fontSize: (big ? 25 : 19) * (T.titleUpper ? 1 : 1.05),
    fontWeight: T.titleWeight,
    letterSpacing: T.titleSpacing,
    lineHeight: 1.18,
    textShadow: T.titleShadow,
    textTransform: T.titleUpper ? "uppercase" : "none",
    margin: 0,
    color: T.text,
  };
  ...
  const btn = (dis?: boolean): React.CSSProperties => ({
    ...
    color: T.btnText,
    fontWeight: 800,
    letterSpacing: T.titleUpper ? 0.5 : 0.2,
    fontSize: big ? 17 : 14.5,
    padding: big ? "15px 18px" : "12px 14px",
    borderRadius: T.btnRadius,
    fontFamily: "inherit",
    opacity: dis ? 0.5 : 1,
    background: dis ? T.muted : T.btnBg,
    boxShadow: dis ? "none" : T.btnShadow,
  });
  ...
  const opt = (sel: boolean): React.CSSProperties => ({
    ...
    alignItems: "center",
    gap: 4,
    padding: T.optionCols === 2 ? "16px 6px" : "12px 4px",
    borderRadius: 16,
    cursor: "pointer",
  ...
  return (
    <div
      style={{
        ...
        padding: big ? "40px 22px" : "36px 16px 20px",
      }}
    >
      <style>{`
        ...
        {confete.map((h, i) => (
          <span
            ...
            style={{
              ...
              bottom: "-50px",
              fontSize: h.size * s,
              animation: `floatUpDate ${h.dur}s linear ${h.delay}s infinite`,
      ...
      <div
        ...
        style={{
          ...
          gap: 14 * s,
          textAlign: "center",
          animation: "popD .45s cubic-bezier(.2,.8,.2,1.05)",
        }}
      >
        {screen === "s1" && (
          <>
            <Personagem src={T.imgAbertura} emoji={T.emojiAbertura} tamanho={175} />
            <div style={card}>
              <h1 style={h1}>
        ...
        {screen === "s2" && (
          <>
            <Personagem src={T.imgSim} emoji={T.emojiSim} tamanho={160} />
            <div style={card}>
        ...
        {screen === "s6" && (
          <>
            <Personagem src={T.imgFinal} emoji={T.emojiFinal} tamanho={175} />
            <div style={card}>
              <h1 style={h1}>
                Tô contigo, {nickShow}. {T.coracao}
              </h1>
...
// ───────── a brincadeira do sim/não ─────────
// O "NÃO" foge do cursor/dedo. Depois de 3 tentativas ele desiste — assim a
// piada tem desfecho em vez de virar frustração, e quem usa teclado ou tem
// "reduzir movimento" ligado não fica travado sem conseguir responder.
// A carinha triste não se repete aqui porque já é o personagem do topo desta tela.
export function SimNao({ T, big = false, onSim }: { T: Tokens; big?: boolean; onSim: () => void }) {
  ...
  return (
    <>
      {desistiu && (
        <p style={{ fontSize: big ? 15 : 13, color: T.muted, fontWeight: 800, margin: "12px 0 0" }}>
          POR FAVOR 🥺👉👈
        </p>
      )}
      <div ref={zone} style={{ position: "relative", display: "flex", gap: 12, marginTop: 16, alignItems: "center", minHeight: big ? 56 : 46 }}>
        <button style={{ ...base, color: T.btnText, background: T.btnBg, boxShadow: T.btnShadow }} onClick={onSim}>
          SIM
        </button>
        <button
          onMouseEnter={fugir}
          onFocus={fugir}
          onClick={fugir}
          aria-label={desistiu ? "Não (desistiu)" : "Não"}
          style={{
            ...base,
            flex: desistiu ? 0.55 : 1,
            padding: desistiu ? "9px 12px" : base.padding,
            fontSize: desistiu ? 12 : base.fontSize,
            color: T.text,
            background: T.colorScheme === "dark" ? "rgba(0,0,0,.24)" : "rgba(0,0,0,.05)",
            border: `1.5px solid ${T.cardBorder}`,
            cursor: desistiu ? "not-allowed" : "pointer",
            opacity: desistiu ? 0.5 : 1,
