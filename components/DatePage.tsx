...
export const DATE_TEMPLATES: Record<DateTemplateId, Tokens> = {
  // Laranja quente, caixa alta, clima de meme — o original.
  sunset: {
    nome: "Pôr do sol",
  ...
  // Pastel claro, minúsculas, clima fofo/adesivo — o oposto do primeiro.
  docinho: {
    nome: "Docinho",
    descricao: "Pastel claro, ursinho e clima fofo",
    bg: "linear-gradient(180deg, #F6EEFF 0%, #FDF2F8 55%, #EAFBF3 100%)",
    cardBg: "#ffffff",
    cardBorder: "rgba(160,120,220,.22)",
    cardShadow: "0 14px 34px rgba(150,110,200,.18)",
    fieldBg: "#FBF7FF",
    text: "#3B2A52",
    muted: "#8B7BA6",
    accent: "#A97BE8",
    accentSoft: "rgba(169,123,232,.16)",
    btnBg: "linear-gradient(180deg,#C79BF5,#A97BE8)",
    btnText: "#fff",
    btnShadow: "0 10px 22px rgba(169,123,232,.4)",
    radius: 26,
    btnRadius: 99,
...
export default function DatePage({
  ...
  const emojiTop: React.CSSProperties = {
    fontSize: 70 * s,
    lineHeight: 1,
    animation: "bobD 3s ease-in-out infinite",
    filter: T.colorScheme === "dark" ? "drop-shadow(0 10px 14px rgba(0,0,0,.35))" : "drop-shadow(0 10px 14px rgba(150,110,200,.3))",
  };
  ...
  return (
    ...
    >
      ...
      >
        {screen === "s1" && (
          <>
            <div style={emojiTop}>{T.emojiHero}</div>
            <div style={card}>
              <h1 style={h1}>
        ...
        {screen === "s2" && (
          <>
            <div style={emojiTop}>{T.emojiSim}</div>
            <div style={card}>
        ...
        {screen === "s6" && (
          <>
            <div style={emojiTop}>{T.emojiFim}</div>
            <div style={card}>
              <h1 style={h1}>
...
export function SimNao({
  ...
  return (
    <>
      {desistiu && (
        <p style={{ fontSize: big ? 15 : 13, color: T.muted, fontWeight: 800, margin: "10px 0 0" }}>
          POR FAVOR 🥺👉👈
        </p>
      )}
      <div
        ref={zone}
        style={{ position: "relative", display: "flex", gap: 12, marginTop: 16, alignItems: "center", minHeight: big ? 56 : 46 }}
      >
